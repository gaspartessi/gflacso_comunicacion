...
class block_gflacso_comunicacion extends block_base {
    ...
    public function get_content() {
        ...
        //$this->config available in all block methods except init().
...
//            $links .= '<a href="'.$this->buildUrl($config->social_ig).'" title="'.$title_ig.'" class="fab fa-algolia" target="_blank"><span>'.$title_ig.'</span></a>';
            $links .= '<a href="'.$this->buildUrl($config->social_ig).'" title="'.$title_ig.'" target="_blank"><i class="fas fa-camera-retro fa-2x"></i><span>'.$title_ig.'</span></a>';
        if (!empty($config->social_fb))
            $links .= '<a href="'.$this->buildUrl($config->social_fb).'" title="'.$title_fb.'" target="_blank"><i class="fab fa-facebook-square fa-2x" ></i><span>'.$title_ig.'</span></a>';
	...
	if (!filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
	    return '<a href="mailto:'.$email.'" title="'.$title.'" class="'.$this->get_additional_class_by_value($style).'"><span>'.$title.'</span></a>';
        // return '<a href="mailto:'.$email.'" title="'.$title.'" ><i class="'.$this->get_additional_class_by_value1($style).'"></i><span>'.$title.'</span></a>';
	}
